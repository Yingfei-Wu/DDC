...
parser.add_argument('--stream', default='motion', type=str)
...
parser.add_argument('--save-fre', default=20, type=int,
                    help='save frequency')
...
def main_worker(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        worker_init_fn=worker_init_fn, pin_memory=False, sampler=train_sampler, drop_last=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_joint, loss_motion, loss_bone, loss_sim, loss_feature, top1_joint, top1_motion, top1_bone = train(train_loader, model, criterion, optimizer, epoch, args, f=log_txt)
        ...
        if args.local_rank in [-1, 0]:
            ...
            writer.add_scalar('loss_feature', loss_feature.avg, global_step=epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, args, f=None):
    ...
    losses_feature = AverageMeter('Loss Feature', ':6.3f')
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, losses_base, losses_mix, losses_mp, losses_rec, losses_sim, losses_feature, top1_joint, top1_motion, top1_bone],
    ...
    for i, (input_v1, input_v2, input_v3, label) in enumerate(train_loader):
        ...
        m = mask_gen(inputs[0]).cuda()

        output1, output2, output3, output4, output5, target, recons_data, loss_sim = model.forward_CCD(inputs[0], inputs[1], inputs[2], local_rank=args.local_rank,mask=m)

        batch_size = output1.size(0)
        # compute loss
        loss_base = criterion(output1, target)
        loss_mix = criterion(output2, target)
        mask_region = (1.0-m).permute(0,4,1,3,2)
        ...
        loss_feature = criterion(output5, target)

        loss = loss_base + loss_mix + loss_sim + loss_rec*40 + loss_mp + loss_feature*0.1
        ...
        losses_feature.update(loss_feature.item(), batch_size)
    ...
    return losses_base, losses_mix, losses_mp, losses_sim, losses_feature, top1_joint, top1_motion, top1_bone
