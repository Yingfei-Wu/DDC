...
class BIGRU(nn.Module):
    ...
    def forward(self, input_tensor, knn_eval=False,all_seq=False,small_prompt=None,st_idx=None):  
        ...
        input_tensor = self.input_norm(input_tensor.permute(0,2,1).contiguous()).permute(0,2,1).contiguous()  

        seq_len = torch.zeros(input_tensor.size(0),dtype=int) + input_tensor.size(1) 

        encoder_hidden = self.encoder(input_tensor, seq_len, all_seq=all_seq)  
        if knn_eval: 
