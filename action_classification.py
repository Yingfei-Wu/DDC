...
import moco.builder_mask_recons
...
parser.add_argument('--savepath', default='checkpoints', type=str,
                    help='where to save the output of linear_eval')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder_mask_recons.MoCo(args.finetune_skeleton_representation, opts.bi_gru_model_args, pretrain=False)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=False, sampler=train_sampler,drop_last=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            }, is_best,filename = args.savepath + '_checkpoint.pth.tar' )
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # compute output
            output = model(images,view='joint')
            loss = criterion(output, target)
...
def sanity_check(state_dict, pretrained_weights):
    ...
    for k in list(state_dict_pre.keys()):
        if k.startswith('module.'):
            state_dict_pre[k[7:]] = state_dict_pre[k]
